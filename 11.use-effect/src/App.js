/*
  *useEffect()
   : 컴포넌트의 lifecycle을 관리하는 리액트 Hook
     컴포넌트가 생성될 때(mount), update될 때, 삭제(unmount)될 때
     이런lifecycle에서 렌더링 될 때마다 특정작업(side Effect)을 실행
      - side Effect : component가 렌더링 된 이후에 비동기로 처리되어야 하는 부수적인 효과들
  * 사용
    타이머, mount된 후 서버에서 데이터를 백그라운드로 가져올 때, ...
